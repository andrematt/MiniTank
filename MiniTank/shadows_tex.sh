...
void pointLight()
{
	...
	vec3 VP = normalize(vec3(gl_LightSource[i].position - gl_ModelViewMatrix * gl_Vertex)); //OK

	float nDotVP, nDotHV;
	{
		// Compute the normal in eye coordinates
		vec3 normal = normalize(gl_NormalMatrix * gl_Normal); //ok
		...
		vec3 halfVector = normalize(VP + vec3(0.0, 0.0, 1.0)); //ok
	...
	// Ambient
	vec4 color = gl_FrontMaterial.ambient * gl_LightSource[i].ambient; //OK

	// Diffuse
	//color += gl_FrontMaterial.diffuse * vec4(1.0, 223.0/255.0, 240.0/255.0, 1.0) * nDotVP; //?? non errore

	// Specular
	//color += gl_FrontMaterial.specular * gl_LightSource[i].specular * pf; //OK
	...
	gl_FrontColor = clamp(color + gl_FrontLightModelProduct.sceneColor, 0.0, 1.0); //OK
}
...
void main(void)
{
	...
	if (is_out(gl_TexCoord[1]) || shadow2DProj(shadowMap, gl_TexCoord[1]).r == 1.0) //non in ombra
		gl_FragColor = orig_color;
	else
	  //gl_FragColor = vec4(vec3(0.0) + orig_color*0.80);	//scheda intel
	  gl_FragColor = vec4(vec3(0.0) + orig_color*0.80, orig_color.a);  //scheda nvidia
